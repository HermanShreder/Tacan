...
def generate_deeplink(exchange, coin):
    if exchange == 'bitget':
        return f"bitget://spot/{coin}_USDT"
    elif exchange == 'gate':
        return f"gateio://trade/{coin}_USDT"
    elif exchange == 'kucoin':
        return f"kucoin://market/{coin}_USDT"
    elif exchange == 'poloniex':
        return f"poloniex://trade/{coin}_USDT"
    elif exchange == 'binance':
        return f"https://www.binance.com/en/trade/{coin}_USDT"
    elif exchange == 'bybit':
        return f"https://www.bybit.com/trade/spot/{coin}/USDT"
    elif exchange == 'okx':
        return f"https://www.okx.com/trade-spot/{coin.lower()}-usdt"
    elif exchange in ['huobi', 'htx']:
        return f"https://www.htx.com/trade/{coin.lower()}_usdt"
    else:
...
async def scan_all_markets():
    ...
    exchanges = {}
    exchange_list = ['gate', 'kucoin', 'bitget', 'mexc', 'htx', 'huobi', 'poloniex', 'bitmart', 'phemex']
    ...
    while True:
        ...
        logger.info(f"✅ Скан #{scan_count} за {scan_time:.1f}с | Спредов: {len(active_spreads)}")
...
async def main():
    ...
    # Запускаем бота (без await внутри run_polling)
