...
TRADE_SIZE_USD = 500          # сумма сделки (однако для проверки ликвидности используем 1000$)
LIQUIDITY_CHECK_USD = 1000    # минимальная глубина стакана для уверенного исполнения
MIN_SPREAD_PCT = 0.5
MAX_SPREAD_PCT = 200.0
MIN_VOLUME_USD = 50000         # 24h объем
...
NETWORKS_INFO = {
    ...
    'PROVE': {'time_min': 5, 'time_max': 10, 'fee': 1.0, 'speed': '🟡', 'recommended': False},
}
...
def generate_deeplink(exchange, coin):
    """Генерирует ссылку, открывающую приложение для избранных бирж, иначе браузер."""
    pair = f"{coin}_USDT".upper()
    lower_coin = coin.lower()
    # Deeplinks для мобильных приложений
    if exchange == 'bitget':
        return f"bitget://spot/{pair}"
    elif exchange == 'gate':
        return f"gateio://trade/{pair}"
    elif exchange == 'kucoin':
        return f"kucoin://market/{pair}"
    elif exchange == 'poloniex':
        return f"poloniex://trade/{pair}"
    # Браузерные ссылки для остальных (включая Binance)
    elif exchange == 'binance':
        return f"https://www.binance.com/en/trade/{coin}_USDT"
    elif exchange == 'bybit':
        return f"https://www.bybit.com/trade/spot/{coin}/USDT"
    elif exchange == 'okx':
        return f"https://www.okx.com/trade-spot/{lower_coin}-usdt"
    elif exchange == 'huobi' or exchange == 'htx':
        return f"https://www.htx.com/trade/{lower_coin}_usdt"
    else:
        return f"https://{exchange}.com/trade/{coin}_USDT"

async def get_order_book_liquidity(exchange, symbol, side, required_usd):
    """Проверяет, есть ли ликвидность на сумму required_usd. Возвращает (avg_price, total_cost, fee) если хватает."""
    try:
...
async def check_common_network(buy_exchange, sell_exchange, coin):
    """Возвращает лучшую общую сеть или None."""
    try:
...
def format_signal(coin, buy_ex, sell_ex, p_buy, p_sell, buy_fee, sell_fee, net_info, net_profit, net_spread, liquidity_ok):
    ...
    liq_status = "✅ ГЛУБИНА ДОСТАТОЧНА" if liquidity_ok else "⚠️ НЕ ХВАТАЕТ ГЛУБИНЫ"
    return (
...
async def stats_command(update, context):
    ...
    stats_msg += f"💎 Всего сигналов: **{total_opp}**\n"
    stats_msg += f"💰 Мат. профит: **${total_profit:.2f}**\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
    stats_msg += "🟢 **ТОП-5 БИРЖ ДЛЯ ПОКУПКИ (где чаще держать USDT):**\n"
    ...
    stats_msg += "\n🔴 **ТОП-5 БИРЖ ДЛЯ ПРОДАЖИ (где чаще держать монеты):**\n"
    ...
    stats_msg += f"\n🔄 **Активных спредов сейчас:** {len(active_spreads)}"
    await update.message.reply_text(stats_msg, parse_mode="Markdown")

async def start_handler(update, context):
    await update.message.reply_text(
        "✅ **Арбитражный бот запущен**\n\n"
        "📌 Проверка ликвидности на $1000 перед каждым сигналом\n"
        "📌 Спред удаляется, если исчезает с бирж\n"
        "📌 Deeplinks для Bitget, Gate, KuCoin, Poloniex\n"
...
async def scan_all_markets():
    ...
    for ex_id in exchange_list:
        try:
            ...
            logger.warning(f"⚠️ {ex_id} пропущена: {str(e)[:80]}")
    logger.info(f"Загружено {len(exchanges)} бирж")
    ...
    while True:
        ...
        logger.info(f"Сканирую {len(all_tickers)} пар")

        fresh_keys = set()

        # 2. Обработка каждой пары
        for symbol, bids_asks in all_tickers.items():
            coin = symbol.split('/')[0]
            buy_list = sorted(bids_asks.items(), key=lambda x: x[1]['ask'])[:5]   # 5 лучших
            sell_list = sorted(bids_asks.items(), key=lambda x: x[1]['bid'], reverse=True)[:5]

            for buy_ex, buy_data in buy_list:
                for sell_ex, sell_data in sell_list:
                    if buy_ex == sell_ex:
                        continue
                    raw_spread = (sell_data['bid'] - buy_data['ask']) / buy_data['ask'] * 100
                    ...
                    # Пересчёт прибыли на сумму TRADE_SIZE_USD
                    b_fee_trade = TRADE_SIZE_USD * exchanges[buy_ex].market(symbol).get('taker', 0.003)
                    s_fee_trade = TRADE_SIZE_USD * exchanges[sell_ex].market(symbol).get('taker', 0.003)
                    ...
                    if net_spread >= MIN_SPREAD_PCT:
                        ...
                        msg = format_signal(coin, buy_ex, sell_ex, p_buy, p_sell,
                                            ...
                                            net_profit, net_spread, liquidity_ok=True)
                        try:
        ...
        # 3. Очистка неактивных спредов
        for k in list(detected_candidates.keys()):
            if k not in fresh_keys:
                del detected_candidates[k]

        to_remove = []
        for k, data in active_spreads.items():
            if k not in fresh_keys:
                last = spread_last_seen.get(k, 0)
                if current_time - last > 45:
                    to_remove.append(k)
        for k in to_remove:
            try:
        ...
        logger.info(f"Цикл завершён за {time.time()-start_time:.2f}с | Активных спредов: {len(active_spreads)}")
